...
if ($result)
{
    ...
    while ($result_arr=mysqli_fetch_assoc($result)) 
    {
        if  ($result_arr['type'] == 1)
        {
            ...
            array_push($content , "<tr><td class='author'>".$result_arr['author']."</td><td class='content'>".$result_arr['content']."</td><td class='editted_content'>".$result_arr['editted_content']."</td>".$attachment."<td class='sent_at'>".$result_arr['sent_at']."</td><td class='editted_at'>".$result_arr['editted_at']."</td><td class='type'>編輯</td></tr>");
        }
    }
